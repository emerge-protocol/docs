...
async def emerge_callback(
    ...
    if status == 'success':
        ...
        await update_user_consent(uid, {
            'reauthorized_at': datetime.now(timezone.utc)
        })
        ...
        return RedirectResponse(f'/connect?error={quote(error_message)}')
...
3. **Consent lifecycle webhook (optional)** - You may receive `consent.given`/`consent.reauthorized` for audit/state updates
4. **Poll export readiness** - Check `GET /export/status/{uid}` and query only when the provider in `sources[]` has `data_ready: true`
...
- They're redirected to the [Data Wallet](/link/data-wallet) (not back to your `redirect_uri`)
