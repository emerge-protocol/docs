...
| `flow_version` | Yes | The flow version used |
...
```
https://yourapp.com/callback?status=success&state=abc123&uid=user_12345&flow_version=lm
```
...
```
https://yourapp.com/callback?status=reauthorized&state=abc123&uid=user_12345&flow_version=lm
```
...
```
https://yourapp.com/callback?status=failure&state=abc123&uid=user_12345&flow_version=lm&error_code=user_failed
```
...
app.get('/emerge/callback', async (req, res) => {
  const { status, state, uid, flow_version, error_code } = req.query;
  ...
  switch (status) {
    case 'success':
      ...
      await saveUserConsent(uid as string, {
        ...
        flowVersion: flow_version as string
      });
