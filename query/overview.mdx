...
|-----------|---------------|----------------|-------------|
| Search History | `/sync/get_search` | `/search` | Google search queries |
| Browsing History | `/sync/get_browsing` | `/browsing` | Chrome browser history |
| YouTube History | `/sync/get_youtube` | `/youtube` | Watched videos |
| Ad Interactions | `/sync/get_ads` | `/ads` | Ad clicks and views |
  ...
  <Tab title="Async (Job-based)">
    **Best for:** Large datasets, multiple users, batch processing
    ...
    const startResponse = await fetch(
      `https://query.emergedata.ai/v1/search?users=${encodeURIComponent(JSON.stringify(['user_1', 'user_2']))}&begin=2024-01-01&end=2024-01-31`,
      { headers: { 'Authorization': `Bearer ${token}` } }
    );
