---
title: "AI-Native Documentation"
description: "Connect AI tools directly to Emerge documentation"
---

These docs are designed for AI-first workflows. Instead of searching the web or copying text, connect your AI tools directly to this documentation via MCP (Model Context Protocol).

## What's possible

<CardGroup cols={2}>
  <Card title="Ask questions" icon="message-question">
    "How do I query user search history?" → AI searches docs and returns the answer with code examples
  </Card>
  <Card title="Generate code" icon="code">
    "Generate Python code to create a signed link" → AI finds SDK page and provides working implementation
  </Card>
  <Card title="Understand concepts" icon="lightbulb">
    "What's the difference between sync and async queries?" → AI explains with context from the docs
  </Card>
  <Card title="Debug issues" icon="bug">
    "Why is my webhook signature invalid?" → AI finds troubleshooting info and suggests fixes
  </Card>
</CardGroup>

## How it works

```mermaid
sequenceDiagram
    participant You
    participant AI Tool
    participant MCP Server
    participant Emerge Docs

    You->>AI Tool: "How do I handle callbacks?"
    AI Tool->>MCP Server: Search docs
    MCP Server->>Emerge Docs: Query content
    Emerge Docs->>MCP Server: Return relevant pages
    MCP Server->>AI Tool: Structured context
    AI Tool->>You: Answer with code examples
```

The MCP server:
1. Receives search queries from your AI tool
2. Searches the documentation semantically
3. Returns relevant content with context
4. AI tool uses this to answer your question

## Benefits over web search

| Web Search | MCP Connection |
|------------|----------------|
| May find outdated content | Always current documentation |
| Results from multiple sources | Single source of truth |
| Generic code examples | Emerge-specific implementations |
| Manual copy/paste | Structured data for AI |

## Supported AI tools

<CardGroup cols={2}>
  <Card title="Claude" icon="message" href="/ai/mcp-docs-setup">
    Anthropic's Claude via desktop app or claude.ai
  </Card>
  <Card title="Cursor" icon="code" href="/ai/mcp-docs-setup">
    AI-powered code editor
  </Card>
  <Card title="VS Code" icon="window" href="/ai/mcp-docs-setup">
    With Copilot or other AI extensions
  </Card>
  <Card title="Claude Code" icon="terminal" href="/ai/mcp-docs-setup">
    Claude CLI for terminal workflows
  </Card>
</CardGroup>

## Quick setup

Add the MCP server to your tool in one step:

<Tabs>
  <Tab title="Cursor">
    Add to `.cursor/mcp.json`:
    ```json
    {
      "mcpServers": {
        "Emerge": {
          "url": "https://docs.emergedata.ai/mcp"
        }
      }
    }
    ```
  </Tab>
  <Tab title="Claude Code">
    ```bash
    claude mcp add emerge https://docs.emergedata.ai/mcp
    ```
  </Tab>
</Tabs>

See [MCP Docs Setup](/ai/mcp-docs-setup) for detailed instructions for all tools.

## Two MCP Servers

Emerge provides two separate MCP servers for different purposes:

<CardGroup cols={2}>
  <Card title="Docs MCP Server" icon="book" href="/ai/mcp-docs-setup">
    **Search and query this documentation**

    - Semantic documentation search
    - API reference lookup
    - Code example retrieval
    - Best for: Learning the API, troubleshooting
  </Card>
  <Card title="Query API MCP Server" icon="database" href="/ai/mcp-query-setup">
    **Query consented user data via AI tools**

    - Search history, browsing, YouTube
    - Session-based credentials
    - Direct data access for AI agents
    - Best for: Building AI applications with user data
  </Card>
</CardGroup>

| Server | Purpose | URL |
|--------|---------|-----|
| **Docs MCP** | Search documentation | `https://docs.emergedata.ai/mcp` |
| **Query API MCP** | Query user data | `https://mcp.emergedata.ai/mcp` |

Both servers use the same Model Context Protocol, but serve completely different use cases.

## One-shot prompt for app builders

Use this prompt in tools like Lovable, Cursor, or Claude Code after connecting MCP:

```text
You are building a full-stack app that integrates Emerge Link + Query.
Requirements:
- Use https://link.emergedata.ai/link/start to start consent.
- Generate HMAC-SHA256 signature over sorted raw params (exclude signature).
- Use signing_secret for HMAC; never expose secrets in the frontend.
- timestamp must be ISO 8601 (new Date().toISOString()).
- Generate random state, store it server-side, verify on callback.
- redirect_uri must be pre-approved in the Control Room.
- Never ask end-users for uid. If you send uid, use your internal user id. Otherwise store the callback uid.
- Query sync endpoints with uid: https://query.emergedata.ai/v1/sync/get_*
- Wait for export.completed webhook or handle empty results with retry + backoff.
Return TypeScript and Python examples with async/await and error handling.
```

## Contextual menu

Every page in these docs includes a contextual menu with one-click AI integrations. Look for the menu icon on any page to:

- Copy as Markdown for pasting into AI chats
- Open directly in Claude or ChatGPT
- Copy the MCP server URL

Learn more in [Contextual Menu](/ai/contextual-menu).
