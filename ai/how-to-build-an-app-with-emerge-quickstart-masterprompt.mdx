...
async function pollExportReady(uid: string): Promise<void> {
...
app.get('/query-search', async (req, res) => {
  try {
    ...
    await pollExportReady(emergeUid);
...
async def poll_export_ready(uid: str) -> None:
    config = load_config()
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for attempt in range(1, max_attempts + 1):
            ...
            if payload.get("data_ready") is True:
                return
...
async def query_search(user_id: str):
    ...
    try:
        await poll_export_ready(emerge_uid)
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.get(
