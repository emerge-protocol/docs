...
- Before coding, ask the user for config fields and confirm any URLs/params against the docs. At minimum ask for: client_id, signing_secret, api_token, redirect_uri, webhook_secret (if using webhooks), flow_config name (if any), and the current Link start URL + Query endpoint URLs from the docs.
