...
The Model Context Protocol (MCP) allows AI tools to search and query documentation directly. Mintlify automatically generates an MCP server for these docs.

## MCP Server URL

```
https://emerge.mintlify.dev/mcp
```
...
<Tabs>
  ...
  <Tab title="Claude Code">
    ...
    You should see:
    ```
    emerge: https://emerge.mintlify.dev/mcp (connected)
    ```
...
<AccordionGroup>
  <Accordion title="AI doesn't use the MCP server">
    ...
    - Verify the URL is exactly `https://emerge.mintlify.dev/mcp`
  </Accordion>
