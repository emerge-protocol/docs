...
- **Same Data**: Returns Query API sync endpoint data in MCP response format

## MCP Server URL

**Production**: `https://mcp.emergedata.ai/mcp`

This URL serves all production Query API MCP requests.
...
See [MCP Tools Reference](/ai/mcp-query-tools) for detailed tool documentation.
...
- **uid**: User reference ID (e.g., `psub_f6789012345678901234abcdef012345`)

Contact your Emerge integration team to obtain credentials.

### 2. Configure Your AI Tool

Add the MCP server URL with credentials to your AI tool configuration:
...
<Tabs>
  ...
  <Tab title="Claude Desktop">
    ...
    3. URL: `https://mcp.emergedata.ai/mcp?auth_token=YOUR_TOKEN&uid=YOUR_UID`
  </Tab>
  <Tab title="Cursor">
...
```json
{
  "url": "https://mcp.emergedata.ai/mcp?auth_token=YOUR_TOKEN&uid=psub_e5f6789012345678901234abcdef0123"
}
```
...
await use_mcp_tool({
  ...
  arguments: {
    ...
    uid: "psub_a1b2c3d4e5f6789012345678901234ab",
    begin: "2024-01-01T00:00:00Z"
  }
});
```
...
See [MCP Tools Reference](/ai/mcp-query-tools) for detailed response schemas.
...
See [Query API Overview](/query/overview) for details.
...
<CardGroup cols={2}>
  <Card title="MCP Setup" icon="gear" href="/ai/mcp-query-setup">
    Configure Claude, Cursor, VS Code
  </Card>
  <Card title="MCP Tools Reference" icon="tools" href="/ai/mcp-query-tools">
